...
import static com.example.authservice.utility.constants.InternationalizationExceptionVariablesConstants.DELETE_USER_EXCEPTION;
...
import com.example.authservice.dto.user.UserRefreshTokenRequest;
import com.example.authservice.dto.user.UserResponse;
import com.example.authservice.enums.PersonType;
import com.example.authservice.exception.custom.DeleteUserException;
...
public class UserServiceImpl implements UserService {
    ...
    public PersonResponse createPerson(PersonRequest personRequest, PersonType role) {
        ...
        try {
            switch (role) {
                case PASSENGER -> assignRole(personId, PASSENGER.getType());
                case DRIVER -> assignRole(personId, DRIVER.getType());
            }
    ...
    @Override
    public AuthResponse refreshToken(UserRefreshTokenRequest refreshRequest) {
    ...
    @Override
    public void logout(UserRefreshTokenRequest logoutRequest) {
        String tokenEndpoint = String.format("%s/realms/%s/protocol/openid-connect/logout",
                keycloakProperties.authServerUrl(),
                keycloakProperties.realm());
        ...
        body.add("client_id", keycloakProperties.adminClientId());
        body.add("client_secret", keycloakProperties.adminClientSecret());
        body.add("refresh_token", logoutRequest.refreshToken());

        HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<>(body, headers);

        try {
            restTemplate.exchange(tokenEndpoint, HttpMethod.POST, request, new ParameterizedTypeReference<>() {});
        } catch (Exception e) {
            throw new LogoutException(LOGOUT_EXCEPTION, e.getMessage());
        }
    }
    ...
    private void deleteUser(UUID userId) {
        ...
        try {
            ...
            throw new DeleteUserException(DELETE_USER_EXCEPTION, e.getMessage());
        }
    }
